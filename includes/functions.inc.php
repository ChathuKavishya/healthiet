...
function emailExists($conn, $email) {
    $sql = "SELECT * FROM healthcare WHERE email = ?;";
...
function createUser($conn, $fname, $lname, $gender, $age, $email, $pwd) {
    ...
    $sql = "INSERT INTO healthcare (firstName, lastName, gender, age, email, password) VALUES (?, ?, ?, ?, ?, ?);";
    $stmt = mysqli_stmt_init($conn);
...
function loginMealPlan($conn,$email,$bmi){
    $emailExists = emailExists( $conn, $email);
    if ($emailExists === false) {
        header("location: ../login.php?error=wronglogin1");
            exit();
...
function emptyInputLoginMealPlan($email, $bmi) {
    $result;
    /*check whether inputs are empty */
    if (empty($email) || empty($bmi)) {
        $result = true;
    }
    else {
        $result = false;
...
function createMealPlanUser($conn, $email,$bmi) {
   

    //$sql = "INSERT INTO healthcare (firstName, lastName, gender, age, email, password) VALUES (?, ?, ?, ?, ?, ?);";
    $sql = "INSERT INTO mealtable (email,bmi) VALUES (?, ?);";
    $stmt = mysqli_stmt_init($conn);
    ...
    mysqli_stmt_bind_param($stmt, 'ss', $email,$bmi);   
    mysqli_stmt_execute($stmt);       
    mysqli_stmt_close($stmt);     
